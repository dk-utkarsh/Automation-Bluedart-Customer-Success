...
def record_thread(message_id, subject, ticket_map):
    # Locked: the mail has already gone out, so losing this record to a concurrent
    # reply scan would mail the courier the same escalation again tomorrow.
    with threads_lock(label="record_thread"):
        return _record_thread(message_id, subject, ticket_map)


def _record_thread(message_id, subject, ticket_map):
...
def run_followups(verbose=True):
    ...
    # Locked: without it a concurrent reply scan can write back a copy loaded before
    # followup_sent_ist was stamped, and the courier is nudged a second time.
    with threads_lock(label="run_followups"):
        return _run_followups(verbose)


def _run_followups(verbose=True):
    threads = load_threads()
    sent = 0
